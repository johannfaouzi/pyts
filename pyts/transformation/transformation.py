...
class PAA(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        ...
        return np.apply_along_axis(paa, 1, X, n_features, window_size,
    ...
    def transform(self, X):
        ...
        if self.quantiles == 'gaussian':
            quantiles = scipy.stats.norm.ppf(
                np.linspace(0, 1, num=self.n_bins + 1)[1:])
        else:
...
class VSM(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        ...
        return np.array([vsm(X[i], n_features, self.window_size,
                         self.numerosity_reduction) for i in range(n_samples)])
